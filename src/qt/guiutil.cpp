...
#include "foxdcoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Foxdcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new FoxdcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new FoxdcoinAddressCheckValidator(parent));
}
...
bool parseFoxdcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no foxdcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("foxdcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FoxdcoinUnits::parse(FoxdcoinUnits::FOXD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFoxdcoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert foxdcoin:// to foxdcoin:
    //
    //    Cannot handle this later, because foxdcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("foxdcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "foxdcoin:");
    }
    QUrl uriInstance(uri);
    return parseFoxdcoinURI(uriInstance, out);
}

QString formatFoxdcoinURI(const SendCoinsRecipient &info)
{
    QString ret = QString("foxdcoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FoxdcoinUnits::format(FoxdcoinUnits::FOXD, info.amount, false, FoxdcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openFoxdcoinConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(FOXDCOIN_CONF_FILENAME);
    ...
    /* Open foxdcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Foxdcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Foxdcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Foxdcoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Foxdcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "foxdcoin.desktop";
    return GetAutostartDir() / strprintf("foxdcoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a foxdcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Foxdcoin\n";
        else
            optionFile << strprintf("Name=Foxdcoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the foxdcoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef foxdcoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (foxdcoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, foxdcoinAppUrl);

    CFRelease(foxdcoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add foxdcoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, foxdcoinAppUrl, nullptr, nullptr);
    }
