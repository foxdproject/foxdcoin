...
const std::string FoxdcoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString FoxdcoinGUI::DEFAULT_WALLET = "~Default";

FoxdcoinGUI::FoxdcoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void FoxdcoinGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Foxdcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and foxdcoin: URIs)"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Foxdcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Foxdcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a foxdcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Foxdcoin command-line options").arg(tr(PACKAGE_NAME)));
...
void FoxdcoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void FoxdcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        /** FOXD START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/foxdcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* foxdcoinLabelLayout = new QVBoxLayout(toolbarWidget);
        foxdcoinLabelLayout->addWidget(label);
        foxdcoinLabelLayout->addWidget(toolbar);
        foxdcoinLabelLayout->setDirection(QBoxLayout::TopToBottom);
        foxdcoinLabelLayout->addStretch(1);
...
void FoxdcoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool FoxdcoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FoxdcoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FoxdcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void FoxdcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void FoxdcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void FoxdcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FoxdcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FoxdcoinGUI::optionsClicked()
{
...
void FoxdcoinGUI::showDebugWindow()
{
...
void FoxdcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void FoxdcoinGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void FoxdcoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void FoxdcoinGUI::openClicked()
{
...
void FoxdcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FoxdcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FoxdcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FoxdcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FoxdcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FoxdcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** FOXD START */
void FoxdcoinGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void FoxdcoinGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void FoxdcoinGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void FoxdcoinGUI::gotoRestrictedAssetsPage()
{
...
void FoxdcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Foxdcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void FoxdcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FoxdcoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void FoxdcoinGUI::updateHeadersSyncProgressLabel()
{
...
void FoxdcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FoxdcoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Foxdcoin"); // default title
...
void FoxdcoinGUI::changeEvent(QEvent *e)
{
...
void FoxdcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FoxdcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FoxdcoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "FOXD")
        msg += tr("Amount: %1\n").arg(FoxdcoinUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(FoxdcoinUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void FoxdcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FoxdcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FoxdcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FoxdcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FoxdcoinGUI::setHDStatus(int hdEnabled)
{
...
void FoxdcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FoxdcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FoxdcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FoxdcoinGUI::detectShutdown()
{
...
void FoxdcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void FoxdcoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void FoxdcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(FoxdcoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void FoxdcoinGUI::subscribeToCoreSignals()
{
...
void FoxdcoinGUI::unsubscribeFromCoreSignals()
{
...
void FoxdcoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void FoxdcoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<FoxdcoinUnits::Unit> units = FoxdcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FoxdcoinUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(FoxdcoinUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(FoxdcoinUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (FoxdcoinUnits::Unit u : FoxdcoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FoxdcoinUnits::name(u)), this);
...
{
    setText(FoxdcoinUnits::name(newUnits));
}
