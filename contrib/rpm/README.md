...
The RPM spec file provided here is for Foxdcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://foxdcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Foxdcoin git repository](https://github.com/FoxdcoinProject/Foxdcoin) but are not part of
...
Currently only `Source10` is used, for the example `foxdcoin.conf` file.
...
in the foxdcoin git repository. At present only an SVG version of the Foxdcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the foxdcoin
...
needed when 0.12.1 is released, a proper fix is already in the Foxdcoin git
master branch.
...
idea to build Foxdcoin Core against a static version of that library compiled
...
The `foxdcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `foxdcoin-server` package. At this point, most RPM based
...
The `foxdcoin-server` package should have SELinux support. How to properly do
...
Foxdcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/foxdcoin/). He has been
packaging Foxdcoin for Fedora at least since 2012.
...
4. I renamed the `foxdcoin` package that contains the Qt GUI to `foxdcoin-core` as
that appears to be how the general population refers to it, in contrast to
`foxdcoin-xt` or `foxdcoin-classic`. I wanted to make sure the general population
