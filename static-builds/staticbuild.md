...
sudo find ${DISTNAME}/bin -type f -executable -exec ../foxdcoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../foxdcoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
cd ~/
sudo rm -rf win64
cp -rf foxdcoin/contrib/windeploy ~/sign/win64
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/foxdcoin-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer foxdcoin-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer foxdcoin-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
#transfer foxdcoin-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/foxdcoin/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/foxdcoinproject/foxdcoin
mkdir ~/foxdcoin/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder foxdcoin/depends/SDKs
cd foxdcoin/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/foxdcoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Foxdcoin-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/foxdcoin-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer foxdcoin-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
